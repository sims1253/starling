...
// localization probes (STARLING_MOSS_L0_STAGE / STARLING_MOSS_DUMP_LAYERS);
// the default hot path is the whole-model graph below.
bool layer_legacy(const MossModel& m, int li, const std::vector<ggml_bf16_t>& xh,
...
bool forward_legacy(const MossModel& m, const std::vector<float>& input, int64_t S,
...
bool debug_probe_active() {
    return std::getenv("STARLING_MOSS_L0_STAGE") ||
           std::getenv("STARLING_MOSS_DUMP_LAYERS");
}
...
DeviceCache* get_device_cache(const MossModel& m, std::string& e) {
...
ggml_tensor* append_layer_new(ggml_context* c, const MossModel& m, int li,
    ...
    //   - per-head (STARLING_MOSS_PERHEAD=1): the proven 16-iteration loop,
    //     identical op sequence to layer_legacy.
    const float scale = 1.0f / std::sqrt((float)D);
    ggml_tensor* joined;
    if (!std::getenv("STARLING_MOSS_PERHEAD")) {
...
PrefillReplayEntry* get_or_build_prefill(const MossModel& m, int64_t S,
                                         std::string& e) {
...
bool forward_prefill_new(const MossModel& m, const std::vector<float>& input,
    ...
    if (!global_backend().is_gpu()) {
        ...
        if (!ok) { e = "MOSS prefill graph failed"; return false; }
        state.length = S;
        return true;
    }
    ...
    if (!pe->graph->compute(logits)) { e = "MOSS prefill replay failed"; return false; }
    state.length = S;
    return true;
}
...
bool forward_decode_new(const MossModel& m, int32_t prev_token, int64_t past,
    ...
    const int64_t S = 1;
    const int64_t K = past + S;
    // STARLING_MOSS_FULLCAP=1 selects full-capacity attention (the captured-
    // graph shape): attention over [0, max_cache) with an additive f32 mask,
    // used to A/B-gate whether padded softmax flips any token.
    const bool fullcap = std::getenv("STARLING_MOSS_FULLCAP") != nullptr;
    ...
    if (!ok) { e = "MOSS decode graph failed"; return false; }
    state.length = past + S;
    return true;
}
...
struct MossKKey { int K;
    bool operator==(const MossKKey& o) const { return K == o.K; } };
struct MossKKeyHash { size_t operator()(const MossKKey& k) const noexcept { return (size_t)k.K; } };
std::unordered_map<MossKKey, std::unique_ptr<MossKStepGraph>, MossKKeyHash> g_moss_kstep_cache;
std::once_flag g_moss_kstep_clearer_once;
void ensure_moss_kstep_clearer() {
    std::call_once(g_moss_kstep_clearer_once, [] {
        register_decode_cache_clearer([] { g_moss_kstep_cache.clear(); });
    });
}
} // namespace
...
int moss_kstep_K() {
    int v = 4;
    if (const char* env = std::getenv("STARLING_MOSS_KSTEP")) {
        int e = std::atoi(env);
        if (e >= 1) v = e;
    }
...
MossKStepGraph* get_or_build_kstep(const MossModel& m, int K, std::string& e) {
    ensure_moss_kstep_clearer();
    MossKKey key{K};
    auto it = g_moss_kstep_cache.find(key);
    if (it != g_moss_kstep_cache.end()) return it->second.get();

    const auto& lc = m.config.llm;
    DeviceCache* dc = get_device_cache(m, e);
    if (!dc) return nullptr;

    auto kg = std::unique_ptr<MossKStepGraph>(new MossKStepGraph());
    kg->K = K;
    kg->start_past = 0;
    ...
    if (!raw->rg) { e = "MOSS K-step graph build failed"; return nullptr; }
    it = g_moss_kstep_cache.emplace(key, std::move(kg)).first;
...
bool run_kstep(const MossModel& m, int32_t& prev, int64_t& past, int K,
               int32_t eos, int max_new_tokens, std::vector<int32_t>& ids,
               bool& hit_eos, std::string& e) {
    MossKStepGraph* kg = get_or_build_kstep(m, K, e);
    if (!kg) return false;
    ...
    for (int j = 0; j < K; ++j) {
        ...
        if (kg->host_pos[(size_t)j] < 0 || kg->host_pos[(size_t)j] >= (int32_t)mc) {
            e = "MOSS K-step position out of bounds (pos=" +
    ...
    if (!kg->rg->compute_with_captures(out)) { e = "MOSS K-step replay failed"; return false; }
...
bool greedy_generate(const MossModel& m, const InputsEmbeds& i,
    ...
    if (!dbg) {
        ...
        const bool use_kstep = global_backend().is_gpu() &&
                               !std::getenv("STARLING_MOSS_NOKSTEP");
        double dec_sum = 0.0; int dec_n = 0;
        if (use_kstep) {
            const int K = moss_kstep_K();
        ...
        if (timing) {
            std::fprintf(stderr, "MOSS_TIMING prefill=%.1fms decode_tokens=%d avg=%.2fms/tok (kstep=%d)\n",
                         t_pf, dec_n, dec_n ? dec_sum / dec_n : 0.0, (int)use_kstep);
        }
    } else {
        // Debug/probe path: per-layer run_graph loop (layer_legacy) so the
        // STARLING_MOSS_L0_STAGE / STARLING_MOSS_DUMP_LAYERS divergence-local
