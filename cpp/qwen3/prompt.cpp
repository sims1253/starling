// prompt.cpp — qwen3 prompt construction + audio-embedding scatter injection.
#include "prompt.hpp"
...
bool build_inputs_embeds(const Qwen3Model& m, const Prompt& p, const AudioEmbeds& a,
    ...
    if (!lib::embed_and_scatter_audio(m.loader, m.config.llm.hidden, p.ids,
                                      ...
                                      a.width, a.n_tokens, emb, "QWEN3", err))
        return false;
