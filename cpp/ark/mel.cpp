...
// max-clamp(dynamic_range=8) and normalize (x+4)/4). Threaded like moss/mel.cpp;
// the per-frame / per-(m,t) loop nests keep the same reduction order so the
// output is bit-identical to the serial path. pocketfft is reentrant.
#include "mel.hpp"
#include <algorithm>
#include <cmath>
#include <complex>
...
bool compute_log_mel(const Config& cfg, const ModelLoader& ml, const float* pcm,
    ...
    if (S < 2) { err = "ARK reflect padding requires at least 2 PCM samples"; return false; }
    ...
    // the baked RoPE cos/sin tables (3000 rows) always cover it; without the cap
    // long audio overruns the table view and aborts. HF applies the same cap.
    ...
    if (!wt || !ft || wt->type != GGML_TYPE_F32 || ft->type != GGML_TYPE_F32) {
        err = "ARK mel constants missing or not F32";
        return false;
    }
    if ((size_t) ggml_nelements(wt) != N || (size_t) ggml_nelements(ft) != M * B) {
        err = "ARK mel constant shape mismatch";
        return false;
    }
    ...
    if (const char* p = std::getenv("STARLING_ARK_MEL_DUMP")) {
