...
ggml_tensor* wb(ggml_context* c, const ArkModel& m, const std::string& n) {
    return clone_weight(c, m.loader, n.c_str());
}
// Linear: y = W x, result bf16.
ggml_tensor* lin(ggml_context* c, const ArkModel& m, ggml_tensor* x, const std::string& n) {
    return bf(c, ggml_mul_mat(c, wb(c, m, n), bf(c, x)));
}
// Linear with bias: y = W x + b, result bf16. ARK's Qwen2.5 q/k/v projections
// carry biases (unlike moss/Qwen3); o/gate/up/down stay bias-free (use `lin`).
// The GEMM exposes F32; add the (bf16) bias in F32 then round to the BF16
// boundary, matching PyTorch's bf16 autocast.
ggml_tensor* lin_bias(ggml_context* c, const ArkModel& m, ggml_tensor* x,
                      const std::string& n) {
    ggml_tensor* y = ggml_mul_mat(c, wb(c, m, n + ".weight"), bf(c, x));
    y = ggml_add(c, ff(c, y), ff(c, wb(c, m, n + ".bias")));
    return bf(c, y);
}
...
ggml_tensor* rms(ggml_context* c, const ArkModel& m, ggml_tensor* x, const std::string& n, float eps) {
...
// STARLING_ARK_L0_STAGE=<stage> makes layer 0 of a 107-token prefill return
// the selected intermediate as the graph's REAL output (no capture side
// channels, which have proven untrustworthy), dumps it to
// <STARLING_ARK_STAGE_DIR>/moss_stage_<stage>.f32 (default /tmp), and exits.
...
L0Stage l0_stage() {
    L0Stage st;
    st.name = std::getenv("STARLING_ARK_L0_STAGE");
    if (const char* d = std::getenv("STARLING_ARK_STAGE_DIR")) st.dir = d;
...
[[noreturn]] void stage_exit(const L0Stage& st, const std::vector<float>& data) {
    ...
    if (FILE* f = std::fopen(fn.c_str(), "wb")) {
        ...
        if (got == want && fc == 0) {
            std::fprintf(stderr, "ARK L0 probe: wrote %s (%zu floats)\n", fn.c_str(), got);
            ok = true;
        } else {
            std::fprintf(stderr, "ARK L0 probe: FAILED to write %s (wrote %zu/%zu floats, fclose=%d)\n",
                         fn.c_str(), got, want, fc);
        }
    } else {
        std::fprintf(stderr, "ARK L0 probe: FAILED to open %s\n", fn.c_str());
    }
    std::exit(ok ? 0 : 1);
}
...
// localization probes (STARLING_ARK_L0_STAGE / STARLING_ARK_DUMP_LAYERS);
// the default hot path is the whole-model graph below.
bool layer_legacy(const ArkModel& m, int li, const std::vector<ggml_bf16_t>& xh,
...
bool forward_legacy(const ArkModel& m, const std::vector<float>& input, int64_t S,
    ...
    for (int li = 0; li < (int)m.config.llm.n_layers; ++li) {
        ...
        if (!layer_legacy(m, li, x, S, state.length, &state.layers[li], o)) {
            e = "ARK LLM layer graph failed at " + std::to_string(li);
            return false;
        }
        ...
        for (size_t z = 0; z < o.x.size(); ++z) {
            if (!std::isfinite(o.x[z])) {
                e = "non-finite ARK hidden at layer " + std::to_string(li) +
                    " index " + std::to_string(z);
                return false;
            }
        }
        // STARLING_ARK_DUMP_LAYERS=<prefix> dumps each
        // layer's output hidden as <prefix>_<li>.f32 (prefill only).
        if (S > 1) {
            if (const char* dp = std::getenv("STARLING_ARK_DUMP_LAYERS")) {
    ...
    if (!ok) e = "ARK lm_head graph failed";
    return ok;
}

// ===========================================================================
// NEW PATH (Wave A) — whole-model graphs + device-resident KV cache.
//
// One prefill graph (one-shot, built per utterance) and one decode-step graph
// per token (one-shot, exact-width KV for A1; captured in A2). All 28 layers
// + final norm + lm_head live in a SINGLE ggml cgraph, so the hidden state
// flows device->device between layers (no host tobf round-trip) and there is
// exactly one graph alloc per step instead of 28.
//
...
// mask) is byte-identical to layer_legacy — this is a pure restructure.
// ===========================================================================

bool debug_probe_active() {
    return std::getenv("STARLING_ARK_L0_STAGE") ||
           std::getenv("STARLING_ARK_DUMP_LAYERS");
}

// Process-global device-resident KV cache + precomputed RoPE tables. One per
// process (single moss config); zeroed at the start of each utterance. The KV
// tensors live in a persistent ggml_context allocated on the backend buffer;
// graphs reference them (and the RoPE tables) as fixed leaves. Freed by the
// registered decode-cache-clearer BEFORE backend teardown.
struct DeviceCache {
    ...
    void zero(ggml_backend_t backend);
    ~DeviceCache() {
...
bool DeviceCache::init(const LlmConfig& lc, ggml_backend_t backend, std::string& e) {
    ...
    // Precompute the RoPE cos/sin tables with the EXACT f32 std::pow-based
    // formula used by layer_legacy, duplicated halves, rounded to bf16.
...
void DeviceCache::zero(ggml_backend_t backend) {
    (void)backend;
...
DeviceCache* get_device_cache(const ArkModel& m, std::string& e) {
...
// (row qi covers keys j <= past+qi). Same constant + layout as layer_legacy.
...
ggml_tensor* append_layer_new(ggml_context* c, const ArkModel& m, int li,
    ...
    // Qwen2.5 q/k/v projections carry biases (no q_norm/k_norm, unlike Qwen3).
    ggml_tensor* q = lin_bias(c, m, n, p + "attn.q");
    ggml_tensor* k = lin_bias(c, m, n, p + "attn.k");
    ggml_tensor* v = lin_bias(c, m, n, p + "attn.v");
    q = ggml_reshape_3d(c, q, D, H, S);
    k = ggml_reshape_3d(c, k, D, KV, S);
    v = ggml_reshape_3d(c, v, D, KV, S);
    q = ggml_cont(c, ggml_permute(c, q, 0, 2, 1, 3));  // [D, S, H]
    ...
    //   - batched (default, A3): one batched mul_mat over all heads using
    //     ggml's native GQA broadcast (ne12 % ne02 == 0, r2 = H/KV). Math per
    //     head is unchanged; produces [hidden, S] via permute+reshape.
    //   - per-head (STARLING_ARK_PERHEAD=1): the proven 16-iteration loop,
    //     identical op sequence to layer_legacy.
    const float scale = 1.0f / std::sqrt((float)D);
    ggml_tensor* joined;
    if (!std::getenv("STARLING_ARK_PERHEAD")) {
...
// Whole-model prefill graph: embeds -> 28 layers -> final norm -> lm_head
// (last token only). One-shot (runs once per utterance). Writes K/V for slots
// [0, S) into the device cache for the decode loop.
//
// B3: on GPU the prefill graph is captured into a per-S ReplayGraph (prompt
...
// (kv_mode=0 cpy into slots [0, S); the captured cpys execute on replay, writing
// the same device cache the decode K-step graph reads). CPU keeps the one-shot
// build. Cached in a bounded LRU (runtime/lru_cache.hpp) of size
// STARLING_REPLAY_CACHE_SIZE (default 16) — without the bound, each distinct
// prompt length would pin its own captured graph + private gallocr until exit
// (the Wave H OOM bug). Cleared via register_decode_cache_clearer.
...
PrefillReplayEntry* get_or_build_prefill(const ArkModel& m, int64_t S,
                                         std::string& e) {
            ...
            entry.graph = std::make_unique<ReplayGraph>(global_backend(),
                [&](ggml_context* c) -> ggml_tensor* {
                    ...
                    ggml_tensor* last = ggml_view_2d(c, x, lc.hidden, 1, x->nb[1],
                                                     (size_t)(S - 1) * x->nb[1]);
                    ggml_tensor* n = rms(c, m, last, "llm.final_norm.weight", lc.rms_norm_eps);
                    return ff(c, ggml_mul_mat(c, wb(c, m, "llm.embed.weight"), n));
                });
            return entry;
        });
}

bool forward_prefill_new(const ArkModel& m, const std::vector<float>& input,
    ...
    DeviceCache* dc = get_device_cache(m, e);
    if (!dc) return false;
    dc->zero(global_backend().handle());  // fresh utterance
    state.length = 0;
    ...
    if (!global_backend().is_gpu()) {
        ...
        if (!ok) { e = "ARK prefill graph failed"; return false; }
        state.length = S;
        return true;
    }
    ...
    if (!pe->graph->compute(logits)) { e = "ARK prefill replay failed"; return false; }
    state.length = S;
    return true;
}

// Whole-model decode-step graph (S=1): embed(prev) -> 28 layers -> lm_head.
// Exact-width KV (one-shot per step for A1; captured in A2). Reads slots
// [0, past), writes slot `past`, attention over [0, past].
bool forward_decode_new(const ArkModel& m, int32_t prev_token, int64_t past,
    ...
    const int64_t S = 1;
    const int64_t K = past + S;
    // STARLING_ARK_FULLCAP=1 selects full-capacity attention (the captured-
    // graph shape): attention over [0, max_cache) with an additive f32 mask,
    // used to A/B-gate whether padded softmax flips any token.
    const bool fullcap = std::getenv("STARLING_ARK_FULLCAP") != nullptr;
    ...
    bool ok = run_graph([&](ggml_context* c) -> ggml_tensor* {
        ...
        ggml_tensor* n = rms(c, m, x, "llm.final_norm.weight", lc.rms_norm_eps);
        return ff(c, ggml_mul_mat(c, wb(c, m, "llm.embed.weight"), n));
    }, logits);
    if (!ok) { e = "ARK decode graph failed"; return false; }
    state.length = past + S;
    return true;
}

// ===========================================================================
// A4: K-step multistep decode (captured ReplayGraph).
//
...
// step decode. This mirrors the proven parakeet K-step design
// (cpp/parakeet/tdt_multistep.cpp).
//
// Graph cache keyed on (start_past, K): decode advances start_past by K each
// invocation, so each utterance touches ceil(decode_tokens/K) distinct graphs;
// they are cached process-globally and reused across reps / same-prompt runs.
// ===========================================================================

struct ArkKStepGraph {
    int K = 0;
...
namespace {
// ONE captured graph per K (start_past is a runtime input, so a single graph
// serves every decode step and every utterance -> perfect capture amortization).
struct ArkKKey { int K;
    bool operator==(const ArkKKey& o) const { return K == o.K; } };
struct ArkKKeyHash { size_t operator()(const ArkKKey& k) const noexcept { return (size_t)k.K; } };
std::unordered_map<ArkKKey, std::unique_ptr<ArkKStepGraph>, ArkKKeyHash> g_ark_kstep_cache;
std::once_flag g_ark_kstep_clearer_once;
void ensure_ark_kstep_clearer() {
    std::call_once(g_ark_kstep_clearer_once, [] {
        register_decode_cache_clearer([] { g_ark_kstep_cache.clear(); });
    });
}
} // namespace
...
int ark_kstep_K() {
    int v = 4;
    if (const char* env = std::getenv("STARLING_ARK_KSTEP")) {
        int e = std::atoi(env);
        if (e >= 1) v = e;
    }
...
// Build (or fetch) the single full-capacity K-step graph for K. start_past is a
// runtime input, so this graph is reused for every decode-step batch.
ArkKStepGraph* get_or_build_kstep(const ArkModel& m, int K, std::string& e) {
    ensure_ark_kstep_clearer();
    ArkKKey key{K};
    auto it = g_ark_kstep_cache.find(key);
    if (it != g_ark_kstep_cache.end()) return it->second.get();

    const auto& lc = m.config.llm;
    DeviceCache* dc = get_device_cache(m, e);
    if (!dc) return nullptr;

    auto kg = std::unique_ptr<ArkKStepGraph>(new ArkKStepGraph());
    kg->K = K;
    kg->start_past = 0;
    ...
    if (!raw->rg) { e = "ARK K-step graph build failed"; return nullptr; }
    it = g_ark_kstep_cache.emplace(key, std::move(kg)).first;
...
bool run_kstep(const ArkModel& m, int32_t& prev, int64_t& past, int K,
               int32_t eos, int max_new_tokens, std::vector<int32_t>& ids,
               bool& hit_eos, std::string& e) {
    ArkKStepGraph* kg = get_or_build_kstep(m, K, e);
    if (!kg) return false;
    ...
    for (int j = 0; j < K; ++j) {
        ...
        // Load-bearing invariant (Wave G): no decode step's device index -- neither
        // the set_rows KV write target nor the get_rows RoPE source -- may reach
        // max_cache. greedy_generate's tail-cap (Kk = min(K, remaining)) makes
        // this hold; this guard turns any future regression of that bound into a
        // detectable error instead of silent device-memory corruption. (The OOB
        // write lands in ggml's buffer padding and does NOT fault, so a
        // crash/CUDA-error gate alone cannot catch this class of bug.)
        if (kg->host_pos[(size_t)j] < 0 || kg->host_pos[(size_t)j] >= (int32_t)mc) {
            e = "ARK K-step position out of bounds (pos=" +
    ...
    if (!kg->rg->compute_with_captures(out)) { e = "ARK K-step replay failed"; return false; }
...
// Current number of cached prefill graphs (diagnostic + the Wave H bounded-LRU
// regression-test hook). Defined at namespace scope (external linkage) so the
// C++ stress test can link it; g_prefill_cache lives in the anonymous namespace
// above and is reachable by unqualified lookup within this TU.
...
bool llm_prefill(const ArkModel& m, const InputsEmbeds& i, int32_t maxc,
    ...
    if (i.n_tokens <= 0 || i.width != (int64_t)m.config.llm.hidden || i.n_tokens > maxc) {
        e = "invalid ARK prefill shape/cache";
        return false;
    }
    ensure_weights_realized(m.loader);
    const bool dbg = debug_probe_active();
    bool ok = dbg ? forward_legacy(m, i.data, i.n_tokens, o.state, o.logits, e)
                  : forward_prefill_new(m, i.data, i.n_tokens, o.state, o.logits, e);
    if (!ok) return false;
    o.first_token = argmax_low(o.logits);
    // STARLING_ARK_DUMP_LOGITS=<file> dumps prefill logits.
    if (const char* fp = std::getenv("STARLING_ARK_DUMP_LOGITS")) {
...
bool greedy_generate(const ArkModel& m, const InputsEmbeds& i,
    ...
    if (i.n_tokens + op.max_new_tokens > op.max_cache_len) {
        e = "ARK generation exceeds cache";
        return false;
    }
    const bool dbg = debug_probe_active();
    ...
    if (!dbg) {
        ...
        if (timing) {
            ...
            bool ok = forward_prefill_new(m, i.data, i.n_tokens, state, o.prefill_logits, e);
            auto t1 = std::chrono::steady_clock::now();
            t_pf = std::chrono::duration<double, std::milli>(t1 - t0).count();
            if (!ok) return false;
        } else {
            if (!forward_prefill_new(m, i.data, i.n_tokens, state, o.prefill_logits, e))
                return false;
        }
        int32_t prev = argmax_low(o.prefill_logits);
        o.ids.push_back(prev);
        const bool use_kstep = global_backend().is_gpu() &&
                               !std::getenv("STARLING_ARK_NOKSTEP");
        double dec_sum = 0.0; int dec_n = 0;
        if (use_kstep) {
            const int K = ark_kstep_K();
            for (int n = 1; n < op.max_new_tokens;) {
                bool hit = false;
                size_t before = o.ids.size();
                // K-step OOB fix (Wave G): cap this block's step count to the
                // remaining token budget. The outer cache check only bounds the
                // LAST NEEDED token position (n_tokens + max_new_tokens - 1 <=
                // max_cache - 1); without this cap, the final K-step block still
                // runs all K decode steps and its wasted tail steps (those past
            ...
            for (int n = 1; n < op.max_new_tokens; ++n) {
                std::vector<float> dl;
                double s0 = timing ? (double)std::chrono::steady_clock::now().time_since_epoch().count() : 0.0;
                if (!forward_decode_new(m, prev, state.length, state, dl, e)) return false;
                prev = argmax_low(dl);
                o.ids.push_back(prev);
                if (timing) {
                    double s1 = (double)std::chrono::steady_clock::now().time_since_epoch().count();
        ...
        if (timing) {
            std::fprintf(stderr, "ARK_TIMING prefill=%.1fms decode_tokens=%d avg=%.2fms/tok (kstep=%d)\n",
                         t_pf, dec_n, dec_n ? dec_sum / dec_n : 0.0, (int)use_kstep);
        }
    } else {
        // Debug/probe path: per-layer run_graph loop (layer_legacy) so the
        // STARLING_ARK_L0_STAGE / STARLING_ARK_DUMP_LAYERS divergence-local
    ...
    // STARLING_ARK_DUMP_IDS=<file> dumps generated ids (i32).
    if (const char* fp = std::getenv("STARLING_ARK_DUMP_IDS")) {
        if (FILE* f = std::fopen(fp, "wb")) {
