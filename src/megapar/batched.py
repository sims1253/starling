...
The batch=1 pipeline (:class:`megapar.pipeline.MegaPipeline`) keeps the RTX 5090
...
class BatchedLLMMega:
    ...
    into a single CUDA graph, mirroring :class:`megapar.llm_mega.LLMMega` but
...
class BatchedFusedLLMMega(BatchedLLMMega):
    ...
    custom forward that mirrors :class:`megapar.llm_mega.FusedLLMMega` but with a
    ...
    bit-for-bit (verified 0.0 diff in :mod:`megapar.llm_kernels`).
    """
