"""Multi-step CUDA-graph capture for the ARK-ASR-3B Qwen2.5 LLM decoder.

The single-step graph decoder (:class:`starling.ark.llm_mega.LLMMega` /
:class:`starling.ark.llm_mega.FusedLLMMega`) captures ONE decode step per graph
and replays it once per emitted token. Each replay is followed by **two
...
Over a 200-token decode that is ~400 syncs and ~800 host-launched ops, each
...
Design (greedy-in-graph, post-hoc EOS trim)
-------------------------------------------
* **Argmax in-graph.** Each captured step runs ``decode_step -> argmax ->
  write output_ids[j] -> copy argmax into static_input_ids``. Step ``j+1`` reads
  step ``j``'s argmax from the same fixed-address buffer. No host sync between
  captured steps.
* **Position / mask advance in-graph.** ``static_position_ids`` and a scalar
  ``valid_len_buf`` are incremented by 1 inside the graph after each step. The
  4D attention mask is advanced each step via a single-element ``index_fill_``
  (device-side, no CPU scalar). Because both advance by exactly K per replay and
  the cache ``cumulative_length`` auto-advances by K too, the state is correct
  for the *next* chunk with **zero host staging**.
* **EOS handling (post-hoc trim).** The captured graph always runs K steps
  ...
  unaffected. This is byte-exact for non-EOS-early-stop decodes and correct
  (matching greedy up to and including EOS) for early-stop cases.
* **Chunked generation.** ``max_new_tokens - 1`` decode steps are served by
  ``ceil(n_decode / K)`` replays. Each replay does ONE sync
  (``output_ids.tolist()``). The last chunk may produce a few surplus tokens;
  these are trimmed to ``max_new_tokens``.

Correctness
-----------
The emitted token sequence is **byte-exact** with the single-step greedy
decoder. Greedy decoding is deterministic; the only behavioural change is
*when* the argmax runs and *when* the host syncs, not the arithmetic.

Public API
----------
``MultiStepLLMMega(language_model, lm_head, max_cache_len=4096, steps_per_replay=16)``
``MultiStepLLMMega.generate(inputs_embeds, max_new_tokens=200, eos_token_id=...)``
    -> :class:`starling.ark.llm_mega.GenerateResult`
"""
...
class MultiStepLLMMega(FusedLLMMega):
    """K-step CUDA-graph-captured greedy decoder for the ARK-ASR-3B LLM.
    ...
    Args:
        language_model: The ``Qwen2Model`` decoder trunk.
        lm_head: ``nn.Linear`` lm_head from the top-level model.
        max_cache_len: Fixed K/V cache length.
        steps_per_replay: Number of consecutive decode steps captured into one
            graph replay (``K``). Larger ``K`` amortises more syncs but captures
            a bigger graph and needs more cache head-room (the last chunk always
            runs full ``K`` steps even if fewer are needed).
    ...
    def _reset_to_chunk_start(
        ...
        * ``static_attn_mask`` -> positions [0, base-1] unmasked, rest masked
        ...
        # Reset the mask: unmask [0, base-1] (from prefill + prior chunks); the
        # in-graph index_fill_ will unmask one new position per step.
    ...
    def _captured_step(self, j: int) -> None:
        ...
        cache ``cumulative_length``), computes one greedy decode step, and writes
        the emitted token into ``output_ids[j]``. ``static_input_ids`` is chained
        in-graph (argmax -> copy) so the next captured step reads the correct
        input token without a host sync. For ``K=1`` this is equivalent to the
        single-step path (same value, byte-identical output).
        """
        # (a) unmask the single new position being written this step. At step
        #     j the cache writes slot ``base + j`` (= valid_len_buf - 1) and the
        #     attention must include it. This is a single-element index_fill_
        #     (one kernel) instead of rebuilding the full M-wide mask each step.
    ...
    def capture(self, first_token: torch.Tensor, prefill_len: int) -> None:
        ...
        Also captures the parent's single-step ``_graph`` so that callers that
        rely on single-token ``llm._graph.replay()`` fallbacks work with a
        ...
        # (0) Capture the single-step graph (parent) for single-token fallback
        ...
        # cleanly at position T. Warmup scribbles garbage K/V into slots [T, ...)
        # which is overwritten before it is ever read (masked out by the 4D mask
        # and overwritten by the real decode).
        ...
        # in-place-mutated buffers left by the previous step (the output_ids
    ...
    def generate(
        ...
        # (3) reset to chunk-0 start (the graph's in-graph increments handle all
        #     subsequent chunk boundaries automatically).
