...
if is_deepspeed_available():
    import importlib
    deepspeed_groups = importlib.import_module("deepspeed.utils").groups
    _SeqAllToAll = importlib.import_module("deepspeed.sequence.layer")._SeqAllToAll
...
def all_gather_tensors(input_, size=None, dim=0, group=None):
    ...
    if isinstance(gathered_tensors, torch.Tensor) and gathered_tensors.is_nested:
        gathered_tensors = list(gathered_tensors.unbind())
    else:
...
### GRAD NORMS & LEARNING RATES
safe_get_full_grad = None
try:
    import importlib as _il
    safe_get_full_grad = _il.import_module("deepspeed.utils").safe_get_full_grad
except Exception:
    pass

def _collect_full_grads(ds_engine, cpu=True):
    """Works for ZeRO-1/2/3. Use safe_get_full_grad when .grad is sharded/None."""
    grads = {}
    with torch.no_grad():
        for name, p in ds_engine.module.named_parameters():
            if not p.requires_grad:
                continue
            g = p.grad  # ZeRO-1 case - not tested!!!
            if g is None and safe_get_full_grad is not None:  # ZeRO-2/3 case
...
def _model_components(model):
    ### ADD MORE IF CHANGE MODEL ARCHITECTURE!!!
    m = model
    comps = {}
    # --- Embeddings ---
    embeddings = []
    if hasattr(m, "embed_tokens"): embeddings.append(m.embed_tokens)
    if hasattr(m, "audio_codebook_embeddings"): embeddings.append(m.audio_codebook_embeddings)
    if embeddings: comps["embeddings"] = embeddings
    # --- LLM main stack ---
    main_layers = []
    if hasattr(m, "layers"): main_layers.append(m.layers)
    if hasattr(m, "norm"): main_layers.append(m.norm)
    if main_layers: comps["llm_main"] = main_layers
    # --- Decoder heads (text/audio) ---
    if hasattr(m, "audio_decoder_proj"): 
        if hasattr(m.audio_decoder_proj, "text_lm_head"):
            comps["llm_text_head"] = [m.audio_decoder_proj.text_lm_head]
        if hasattr(m.audio_decoder_proj, "audio_lm_head"):
            comps["llm_audio_head"] = [m.audio_decoder_proj.audio_lm_head]
    # --- Audio tower & projector ---
    if hasattr(m, "audio_tower"):
        comps["audio_tower"] = [m.audio_tower]
    if hasattr(m, "audio_encoder_proj"):
        comps["audio_encoder_proj"] = [m.audio_encoder_proj]
    return comps

def _named_params_unique(module):
    """Yield (name, param) without double-counting shared params."""
    seen = set()
...
def _build_group_param_ids(model):
    """
    group_param_ids: {group -> set(param_id)}
    group_param_counts: {group -> (#params, #trainable)}
    """
    comps = _model_components(model)
    group_param_ids = {}
    group_param_counts = {}
    for g, modules in comps.items():
        ids = set()
        n_total, n_trainable = 0, 0
        for mod in modules:
            for _, p in _named_params_unique(mod):
                pid = id(p)
                if pid in ids:
                    continue
                ids.add(pid)
                ...
                if p.requires_grad:
                    n_trainable += p.numel()
        group_param_ids[g] = ids
        group_param_counts[g] = (n_total, n_trainable)
    return group_param_ids, group_param_counts

def _parse_kv_floats(s: str):
    out = {}
    if not s:
        return out
    for pair in s.split(","):
        ...
        if not pair:
            continue
        if ":" not in pair:
            raise ValueError(f"Invalid kv pair (missing ':'): {pair}")
        k, v = pair.split(":", 1)
        out[k.strip()] = float(v.strip())
    return out

def log_grouped_grad_norms_from_grads(
    grads_by_name,         # {param_name: full, averaged grad tensor} from safe_get_full_grad
    accelerator,
    group_param_ids,       # from _build_group_param_ids(model)[0]
    step: int,
    prefix: str = "train/grad_norm",
):
    if not accelerator.is_main_process: return {}, 0.0
    if not grads_by_name: return {}, 0.0
    name_to_id = {n: id(p) for n, p in _named_params_unique(accelerator.deepspeed_engine_wrapped.engine.module)}
    group_sumsq = {g: 0.0 for g in group_param_ids}
    total_sumsq = 0.0
    for pname, g in grads_by_name.items():
        pid = name_to_id.get(pname)
        if pid is None or g is None:
            continue
        # full, averaged grad -> compute L2^2
        val = torch.linalg.vector_norm(g.detach().float().cpu()).item()
        ss = val * val
        total_sumsq += ss
        for grp, idset in group_param_ids.items():
            if pid in idset:
                group_sumsq[grp] += ss
    total_norm = math.sqrt(max(total_sumsq, 0.0))
    group_norms = {g: math.sqrt(max(ss, 0.0)) for g, ss in group_sumsq.items()}
    accelerator.log({f"{prefix}_from_components_total": total_norm}, step=step)
    for g, nrm in group_norms.items():
        accelerator.log({f"{prefix}/{g}": nrm}, step=step)
    return group_norms, total_norm

def log_grouped_lrs(accelerator, optimizer, step, plan):
    # accumulate lr * numel per component
    comp_lr_num = {c: 0.0 for c in plan["comp_total"]}
    for i, pg in enumerate(optimizer.param_groups):
        comp = plan["group_to_comp"][i]
        n    = plan["group_numel"][i]
        comp_lr_num[comp] += pg["lr"] * n
    # per-component (merged) LR
    for comp, tot_n in plan["comp_total"].items():
        lr_c = comp_lr_num[comp] / (tot_n or 1)
        accelerator.log({f"train/lr/{comp}": lr_c}, step=step)
    # overall weighted LR
