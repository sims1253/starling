"""Vendored copy of ``boson_multimodal.data_collator.higgs_audio_collator``.

``boson-multimodal`` pins ``transformers==4.46.3`` and its bundled
``model.higgs_audio.modeling`` imports ``LLAMA_ATTENTION_CLASSES`` (removed in
transformers 5.x), which is incompatible with this repo's pinned
``transformers 5.13.0.dev0``.  The model itself loads fine via
``trust_remote_code`` (its own remote modeling has no such import), so we only
need the *collator* from ``boson_multimodal`` -- and that file has just two
incompatible imports, both replaceable:

* ``ChatMLDatasetSample`` -- a plain ``@dataclass`` (reproduced below verbatim
  from ``boson_multimodal.dataset.chatml_dataset``).
* ``build_delay_pattern_mask`` -- only referenced inside the
  ``use_delay_pattern=True`` branches.  The v3 STT checkpoint has
  ``use_delay_pattern=False`` (see ``config.json``), so the stub below is never
  called at runtime; it raises loudly if it ever is, so a future caller can't
  silently get wrong results.

Everything else is byte-for-byte the upstream collator so prefill embeddings /
masks are identical to the reference ``transcribe()`` path.
"""
...
# --- vendored dataclass (was ``boson_multimodal.dataset.chatml_dataset``) ----
from .chatml_dataset import ChatMLDatasetSample  # noqa: E402


def build_delay_pattern_mask(*args, **kwargs):  # noqa: D401
    """Stub for the upstream delay-pattern helper.

    Unused for the v3 STT checkpoint (``use_delay_pattern=False``).  Raises if a
    caller ever hits a code path that needs it, so misconfiguration is loud.
    """
    raise NotImplementedError(
        "build_delay_pattern_mask is not vendored: the v3 STT checkpoint uses "
        "use_delay_pattern=False. If you need delay-pattern audio-out decoding, "
        "install a transformers-4.x-compatible boson_multimodal in an isolated env."
    )
