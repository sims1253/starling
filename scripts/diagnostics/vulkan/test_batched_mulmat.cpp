...
// is distinguishable from uniform numeric drift.
static void cmp(const char* name, int n_batch, auto build) {
...
int main() {
    ...
    cmp("kq bf16 D128 K107 S107 KV8 H16", 16, [](ggml_context* c) {
    ...
    cmp("kq f32  D128 K107 S107 KV8 H16", 16, [](ggml_context* c) {
    ...
    cmp("vt bf16 (perm+cont) pr bf16", 16, [](ggml_context* c) {
    ...
    cmp("kq bf16 PER-HEAD view2d (h=3)", 1, [](ggml_context* c) {
        ggml_tensor* k = ggml_new_tensor_3d(c, GGML_TYPE_BF16, 128, 107, 8);
        ggml_tensor* q = ggml_new_tensor_3d(c, GGML_TYPE_BF16, 128, 107, 16);
    ...
    cmp("kq bf16 CACHE-VIEW src0 (kv0=0)", 16, [](ggml_context* c) {
    ...
    cmp("kq f32  CACHE-VIEW src0", 16, [](ggml_context* c) {
        ggml_tensor* kc = ggml_new_tensor_3d(c, GGML_TYPE_F32, 128, 512, 8);
        ggml_tensor* k = ggml_view_3d(c, kc, 128, 107, 8, kc->nb[1], kc->nb[2], 0);
        ggml_tensor* q = ggml_new_tensor_3d(c, GGML_TYPE_F32, 128, 107, 16);
        return ggml_mul_mat(c, k, q); });

    cmp("kq f16  CACHE-VIEW src0", 16, [](ggml_context* c) {
    ...
    cmp("kq bf16 CACHE-VIEW src1", 16, [](ggml_context* c) {
        ggml_tensor* k = ggml_new_tensor_3d(c, GGML_TYPE_BF16, 128, 107, 8);
