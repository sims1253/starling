...
def main() -> int:
    ...
    for fx in ["short", "medium", "long"]:
        ...
        pipe.llm.generate(ie, max_new_tokens=8, eos_token_id=EOS_TOKEN_ID)
        torch.cuda.synchronize()
        ...
        res = pipe.llm.generate(ie, max_new_tokens=200, eos_token_id=EOS_TOKEN_ID)
        torch.cuda.synchronize()
        ...
        print(f"         decoded {res.n_tokens} tokens; golden {golden.get(fx, {}).get('ms', '?')} ms")
